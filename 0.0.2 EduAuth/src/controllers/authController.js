...
const forgotPassword = async (req, res) => {
  ...
  try {
      ...
      const user = await findUserByEmail(email); //User.findOne({ email });
      if (!user) {
...
const resetPassword = async (req, res) => {
  ...
  try {
    console.log(token);
      ...
      // Find the user
      const user = await findUserByEmail(email); //User.findOne({
          //_id: decoded.id//,
      ...
      const updatePass = await updatePassword(user._id,newPassword );
